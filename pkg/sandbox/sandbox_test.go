...
func TestSandboxRunner_Prepare_FlavourDetection(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wsDir := t.TempDir()
			for _, f := range tt.markerFiles {
				if err := os.WriteFile(fmt.Sprintf("%s/%s", wsDir, f), []byte{}, 0o644); err != nil {
			...
			runner := NewSandboxRunner(SandboxDeps{
				...
				EnvProvider:     &mockEnvProvider{},
				Logger:          testLogger(),
				FlavourDetector: &mockFlavourDetector{},
			...
			sb, err := runner.Prepare(context.Background(), "test-agent", RunOpts{
				...
				EgressProfile: string(egress.ProfilePermissive),
				Flavour:       tt.flavour,
				ImageOverride: tt.imageOverride,
			})
			require.NoError(t, err)
			defer func() { _ = sb.Cleanup() }()

			assert.Equal(t, tt.wantImage, vmRunner.startCfg.Image)
		})
	}
}

// mockFlavourDetector delegates to the real FileDetector for convenience.
type mockFlavourDetector struct{}

func (m *mockFlavourDetector) Detect(ctx context.Context, workspacePath string) (flavour.Detection, error) {
	// Use real file detection for test simplicity.
	for _, rule := range []struct {
		name  flavour.Name
		files []string
	}{
		{flavour.Go, []string{"go.mod"}},
		{flavour.Node, []string{"package.json"}},
		{flavour.Python, []string{"pyproject.toml", "requirements.txt", "setup.py"}},
		{flavour.Rust, []string{"Cargo.toml"}},
	} {
		for _, f := range rule.files {
			if _, err := os.Stat(fmt.Sprintf("%s/%s", workspacePath, f)); err == nil {
				return flavour.Detection{Primary: rule.name}, nil
			}
		}
	}
	return flavour.Detection{Primary: flavour.Generic}, nil
}

// mockImageResolver uses the same convention as the real resolver.
// Uses last-slash-aware splitting to handle port-bearing registries correctly.
type mockImageResolver struct{}

func (m *mockImageResolver) Resolve(agentImage string, f flavour.Name) string {
	if f == flavour.Generic || f == "" || !f.IsValid() {
		return agentImage
	}
	// Split on the last slash to isolate image name from registry prefix,
	// then find the colon only in the image name portion.
	lastSlash := strings.LastIndex(agentImage, "/")
	prefix := ""
	nameAndTag := agentImage
	if lastSlash >= 0 {
		prefix = agentImage[:lastSlash+1]
		nameAndTag = agentImage[lastSlash+1:]
	}
	if idx := strings.LastIndex(nameAndTag, ":"); idx >= 0 {
		return prefix + nameAndTag[:idx] + "-" + string(f) + ":" + nameAndTag[idx+1:]
	}
	return agentImage + "-" + string(f)
}

// errDetector always returns an error from Detect.
type errDetector struct{ err error }

func (d *errDetector) Detect(_ context.Context, _ string) (flavour.Detection, error) {
	return flavour.Detection{}, d.err
}

func TestSandboxRunner_Prepare_FlavourDetection_InvalidOverride(t *testing.T) {
	t.Parallel()
	...
	// Put a go.mod so auto-detection would find Go.
	require.NoError(t, os.WriteFile(fmt.Sprintf("%s/go.mod", wsDir), []byte{}, 0o644))
	...
	// Invalid flavour should fall through to auto-detection.
	sb, err := runner.Prepare(context.Background(), "test-agent", RunOpts{
		Workspace:     wsDir,
		EgressProfile: string(egress.ProfilePermissive),
		Flavour:       "java",
...
func TestSandboxRunner_Prepare_FlavourDetection_DetectorError(t *testing.T) {
	...
	runner := NewSandboxRunner(SandboxDeps{
		...
		EnvProvider:     &mockEnvProvider{},
		Logger:          testLogger(),
		FlavourDetector: &errDetector{err: fmt.Errorf("disk I/O error")},
		ImageResolver:   &mockImageResolver{},
	})

	// Detector error should gracefully fall back to original image.
	sb, err := runner.Prepare(context.Background(), "test-agent", RunOpts{
		Workspace:     t.TempDir(),
		EgressProfile: string(egress.ProfilePermissive),
		Flavour:       "auto",
